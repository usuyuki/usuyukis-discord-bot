...
func helpMessage(botID, botName string) string {
	...
	return fmt.Sprintf(
		"%sの機能一覧:\n"+
			...
			"- チャンネル作成: `%s channel create <チャンネル名>` でチャンネル作成を提案する（名前は半角英小文字・数字・ハイフン・アンダースコアのみ）。提案メッセージへのリアクションが必要人数（サーバーごとに管理画面で設定、デフォルト2人。提案者自身の分も含む）に達するとBotが公開テキストチャンネルを代理作成する\n"+
