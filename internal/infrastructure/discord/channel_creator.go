...
// CreateTextChannel はguildIDに公開テキストチャンネルnameを作成する。
// カテゴリを指定しないため@everyoneの閲覧権限を含むギルドのデフォルト権限をそのまま継承する
func (c *ChannelCreator) CreateTextChannel(_ context.Context, guildID, name string) error {
	_, err := c.session.GuildChannelCreate(guildID, name, discordgo.ChannelTypeGuildText)
	if err != nil {
