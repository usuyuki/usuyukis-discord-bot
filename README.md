...
| 非公開チャンネル保護 | 一般ユーザーにもチャンネル作成権限（`チャンネルの管理`ロール）を付与する運用を前提に、誰かが非公開チャンネルを作成すると、そのチャンネルに限定してチャンネル管理ロールのアクセスを自動的に剥奪し、作成者本人とサーバー管理者のみが操作できる状態にする（詳細は [adr/0012_private_channel_protection.md](./adr/0012_private_channel_protection.md)） |
...
4. 「OAuth2 > URL Generator」で `bot` スコープと以下の権限を選択し、生成されたURLからサーバーへ招待
   ...
   - `Administrator`（非公開チャンネル保護機能に必須。一般ユーザーが作成した直後の非公開チャンネルはBotがまだ閲覧・編集権限を持たないため、チャンネル単位の権限オーバーライドを常にバイパスするAdministrator権限がないと保護のための権限変更自体が行えない。また作成者解決に使う監査ログ閲覧にも必要）

### 2. 一般ユーザーへのチャンネル作成権限付与（非公開チャンネル保護機能を使う場合）

一般ユーザーにもチャンネル作成を許可したい場合、Bot側の設定ではなく**Discordのサーバー設定でロールに直接権限を付与する**。

1. サーバー設定 → ロール で、チャンネル作成を許可したい既存ロール（例: `@メンバー`など）を開く
2. 権限一覧から `チャンネルの管理`（Manage Channels）をONにする
3. 対象のロールを複数に分けて運用している場合（例: `role0`, `role1`, `role2`のうち`role0`と`role1`だけに許可したい場合）も、新しいロールを作る必要はなく、それぞれのロールに対して同様に`チャンネルの管理`をONにするだけでよい

これだけだと`チャンネルの管理`はギルド全体に及ぶ権限のため、対象ロールを持つメンバーは他人が作った非公開チャンネルまで閲覧・編集できてしまう。この問題への対処（新規作成されたチャンネルが非公開であれば、その場でチャンネル限定にアクセスを剥奪する）はBot側で自動的に行われるため、追加の設定は不要。Bot は `チャンネルの管理` を持つ全ロールを都度動的に検出して対象にするため、ロール名や数が変わっても設定変更なしに追従する（詳細は [adr/0012_private_channel_protection.md](./adr/0012_private_channel_protection.md)）。
...
動作確認用サーバーなどで特定チャンネル以外への誤爆を防ぎたい場合は `DEV_MODE=true` と `DEV_CHANNEL_ID`（反応させたいチャンネルのID）を設定する。dev mode有効時は指定チャンネル以外の投稿にBotの全ハンドラが一切反応しなくなる。絵文字追加通知・非公開チャンネル保護はいずれも特定チャンネルの投稿ではなくギルド単位のイベントに反応する機能のため、dev mode有効時はどちらも全ギルドで停止する。
