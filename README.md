...
| チャンネル作成（投票制） | `@Bot channel create <チャンネル名>` でチャンネル作成を提案すると、Botが提案メッセージを送信し自ら✅リアクションを付ける。ギルドごとに設定した必要承認人数（デフォルト2人、提案者自身のリアクションも含む）分のユニークユーザーがこのメッセージにリアクション（絵文字の種類は問わない）すると、Bot自身の権限で公開テキストチャンネルを代理作成し、提案元チャンネルへ作成完了を通知する（チャンネル名は半角英小文字・数字・ハイフン・アンダースコア・日本語（ひらがな・カタカナ・漢字）のみ）。一般ユーザーへ`チャンネルの管理`権限を付与する運用は他人の非公開チャンネルまで操作できてしまう問題があり、また即時作成はチャンネルの乱立を招くため、この投票制フローを採用している（詳細は [adr/0013_channel_creation_bot_command.md](./adr/0013_channel_creation_bot_command.md), [adr/0014_channel_creation_approval_voting.md](./adr/0014_channel_creation_approval_voting.md)）。必要承認人数は管理画面のギルド詳細ページから変更できる |
| ヘルプ表示 | `@Bot help` または `@Bot usage` とメンションすると、この機能一覧をチャット上に返信する |
| 管理画面 | キーワード・通知先チャンネル・チャンネル作成の必要承認人数をブラウザから編集できる簡易CMS（認証なし・localhost限定） |
...
internal/
  domain/                        … 値オブジェクト・純粋ロジック（keyword, notifychannel, haiku, channel）
...
4. 「OAuth2 > URL Generator」で `bot` スコープと以下の権限を選択し、生成されたURLからサーバーへ招待
   ...
   - `Manage Channels`（チャンネル作成機能に必須。Bot自身がこの権限でチャンネルを代理作成する）
...
動作確認用サーバーなどで特定チャンネル以外への誤爆を防ぎたい場合は `DEV_MODE=true` と `DEV_CHANNEL_ID`（反応させたいチャンネルのID）を設定する。dev mode有効時は指定チャンネル以外の投稿・リアクションにBotの全ハンドラが一切反応しなくなる。絵文字追加通知はチャンネルを持たないギルド単位のイベントに反応する機能のため、dev mode有効時は全ギルドで停止する。
